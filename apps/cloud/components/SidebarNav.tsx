...
export default function SidebarNav({ orgName, isAdmin = false }: { orgName: string; isAdmin?: boolean }) {
  ...
  return (
    <aside className="fixed left-0 top-0 h-screen w-56 flex flex-col bg-[#213448] border-r border-white/5 z-20">
      {/* Logo */}
      <div className="px-4 py-5 border-b border-white/5">
        <div className="flex items-center gap-2.5">
          <div className="w-8 h-8 rounded-lg bg-gradient-to-br from-[#00ffc3] to-[#00ffc3] flex items-center justify-center flex-shrink-0 shadow-[0_0_16px_rgba(109,94,252,0.35)]">
          ...
          <div className="min-w-0">
            <div className="text-xs font-bold text-white truncate leading-tight">LiveStat</div>
          ...
          return (
            <Link
              ...
              className={`flex items-center gap-3 px-3 py-2.5 rounded-lg text-sm font-medium transition-all ${
                active
                  ? 'bg-[#00ffc3]/15 text-[#00ffc3] shadow-[inset_0_0_0_1px_rgba(109,94,252,0.2)]'
        ...
        {isAdmin && (
          <>
            <div className="pt-3 pb-1 px-3">
              <div className="text-[10px] font-bold uppercase tracking-widest text-[#8b8da6]/50">System</div>
            </div>
            <Link
              href="/admin"
              className={`flex items-center gap-3 px-3 py-2.5 rounded-lg text-sm font-medium transition-all ${
                isActive('/admin')
                  ? 'bg-[#ff4e4e]/15 text-[#ff4e4e] shadow-[inset_0_0_0_1px_rgba(255,78,78,0.2)]'
                  : 'text-[#8b8da6] hover:text-white hover:bg-white/5'
              }`}
            >
      ...
      <div className="px-2 py-4 border-t border-white/5">
        <button
          onClick={signOut}
          className="flex items-center gap-3 w-full px-3 py-2.5 rounded-lg text-sm font-medium text-[#8b8da6] hover:text-white hover:bg-white/5 transition-all"
        >
          <svg width="18" height="18" viewBox="0 0 18 18" fill="none">
