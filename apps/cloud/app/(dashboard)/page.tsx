  ...
  return (
    <div className="p-8 max-w-5xl">
      ...
      <div className="grid grid-cols-2 gap-4 mb-8">
        {stats.map((s) => (
          <Link
            key={s.label}
            href={s.href}
            className="bg-[#213448] border border-white/10 rounded-2xl p-5 hover:border-white/20 transition-colors group"
          >
            <div className="text-4xl font-black tabular-nums" style={{ color: s.color }}>
              {s.value}
            </div>
            <div className="text-[#8b8da6] text-sm mt-1 group-hover:text-white/70 transition-colors">
              {s.label}
            </div>
          </Link>
        ))}
      ...
      <div className="grid grid-cols-3 gap-3 mb-8">
        {[
          { label: 'New Tournament', href: '/tournaments/new', icon: '+', desc: 'Create and configure a new tournament' },
          { label: 'Manage Teams', href: '/teams', icon: '⬡', desc: 'Add teams and player IDs' },
          { label: 'Org Settings', href: '/settings', icon: '◈', desc: 'Customize branding & theme' },
        ].map((a) => (
          <Link
            key={a.label}
            href={a.href}
            className="bg-[#213448] border border-white/10 rounded-xl p-4 hover:border-[#00ffc3]/40 hover:bg-[#213448]/80 transition-all group"
          >
      ...
      {recentTournaments && recentTournaments.length > 0 && (
        <div>
          <div className="flex items-center justify-between mb-3">
            ...
            <Link href="/tournaments" className="text-xs text-[#00ffc3] hover:text-[#8b7ffe] transition-colors">
              View all →
            </Link>
          </div>
      ...
      {!recentTournaments?.length && (
        <div className="bg-[#213448] border border-dashed border-white/10 rounded-2xl p-10 text-center">
