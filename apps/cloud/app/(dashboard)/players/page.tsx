...
export default function PlayersPage() {
  ...
  useEffect(() => {
    async function load() {
      ...
      const [{ data: ts }, { data: tourneys }] = await Promise.all([
      ...
      const teamIds = (ts ?? []).map((t: any) => t.id);

      if (teamIds.length > 0) {
        const { data: ps } = await supabase.from('players').select('*').in('team_id', teamIds).order('display_name');
        setPlayers(ps ?? []);
      }
      setLoading(false);
    }
