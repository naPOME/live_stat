...
export default function MatchesPage() {
  ...
  const [stageMap, setStageMap] = useState<Map<string, { id: string; name: string; tournament_id: string }>>(new Map());
  const [tournamentMap, setTournamentMap] = useState<Map<string, { id: string; name: string }>>(new Map());
  ...
  useEffect(() => {
    async function load() {
      ...
      if (!profile?.org_id) { router.push('/'); return; }

      const { data: tournaments } = await supabase.from('tournaments').select('id, name').eq('org_id', profile.org_id);
      const tournamentIds = (tournaments ?? []).map((t) => t.id);
      setTournamentMap(new Map((tournaments ?? []).map((t) => [t.id, t])));

      if (tournamentIds.length === 0) { setLoading(false); return; }

      const { data: stages } = await supabase.from('stages').select('id, name, tournament_id').in('tournament_id', tournamentIds);
      const stageIds = (stages ?? []).map((s) => s.id);
      setStageMap(new Map((stages ?? []).map((s) => [s.id, s])));

      if (stageIds.length === 0) { setLoading(false); return; }

      const { data: ms } = await supabase.from('matches').select('*').in('stage_id', stageIds).order('created_at', { ascending: false });
  ...
  if (loading) {
    return <div className="p-10 flex items-center justify-center min-h-[50vh]"><span className="loader" /></div>;
  }
