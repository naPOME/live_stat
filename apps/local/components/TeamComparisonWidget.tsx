    ...
    return (
      <div style={{ marginBottom: 32 }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 12 }}>
          <div style={{ fontFamily: "'Montserrat', sans-serif", fontSize: 24, fontWeight: 800, color: aBetter ? p.accent : p.text, minWidth: 60 }}>{format(valA)}</div>
          <div style={{ fontSize: 13, fontWeight: 700, color: p.textMuted, letterSpacing: '0.15em', textTransform: 'uppercase' }}>{label}</div>
          <div style={{ fontFamily: "'Montserrat', sans-serif", fontSize: 24, fontWeight: 800, color: bBetter ? p.accent : p.text, minWidth: 60, textAlign: 'right' }}>{format(valB)}</div>
        </div>
        ...
        <div style={{ maxWidth: 1000, margin: '0 auto' }}>
          ...
          <div style={{ display: 'flex', alignItems: 'flex-end', justifyContent: 'space-between', marginBottom: 40, paddingBottom: 20, borderBottom: `2px solid ${p.separator}` }}>
            <div>
              ...
              <h1 style={{ fontFamily: "'Montserrat', sans-serif", fontSize: 'clamp(36px, 5vw, 56px)', fontWeight: 800, color: p.headerText, margin: 0, lineHeight: 1, letterSpacing: '-0.02em', textTransform: 'uppercase' }}>
                Head To Head
              </h1>
            </div>
            ...
            <div style={{ position: 'relative', padding: '48px', display: 'flex', justifyContent: 'space-between', alignItems: 'flex-end', borderBottom: `1px solid ${p.separator}` }}>
              ...
              <div style={{ position: 'relative', zIndex: 10, display: 'flex', flexDirection: 'column', alignItems: 'flex-start' }}>
                {teamA.logoUrl ? (
                  ...
                  <div style={{ width: 120, height: 120, borderRadius: 24, background: `${p.accent}15`, border: `2px solid ${p.accent}40`, display: 'flex', alignItems: 'center', justifyContent: 'center', fontFamily: "'Montserrat', sans-serif", fontSize: 40, fontWeight: 800, color: p.accent, marginBottom: 16, boxShadow: `0 10px 30px ${p.bg}` }}>
                    {teamA.name.substring(0,3).toUpperCase()}
                  </div>
                )}
                <div style={{ fontFamily: "'Montserrat', sans-serif", fontSize: 28, fontWeight: 800, color: p.text, textTransform: 'uppercase', textShadow: `0 4px 12px ${p.cardBg}` }}>{teamA.name}</div>
              </div>

              {/* VS Marker */}
              <div style={{ position: 'relative', zIndex: 10, paddingBottom: 60, fontFamily: "'Montserrat', sans-serif", fontSize: 48, fontWeight: 800, color: p.textMuted, opacity: 0.5, letterSpacing: '-0.05em', textShadow: `0 4px 12px ${p.cardBg}` }}>
                VS
              </div>
              ...
              <div style={{ position: 'relative', zIndex: 10, display: 'flex', flexDirection: 'column', alignItems: 'flex-end' }}>
                ...
                <div style={{ fontFamily: "'Montserrat', sans-serif", fontSize: 28, fontWeight: 800, color: p.text, textTransform: 'uppercase', textShadow: `0 4px 12px ${p.cardBg}` }}>{teamB.name}</div>
              </div>
