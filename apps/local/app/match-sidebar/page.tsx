...
export default function MatchSidebarPage() {
  ...
  return (
    <div style={{ 
      ...
      {/* Palette Picker Bar - Positioned at bottom right to stay out of the way */}
      <div style={{
        position: 'absolute', bottom: 48, right: 48, zIndex: 50,
        background: 'rgba(0,0,0,0.8)', border: '1px solid rgba(255,255,255,0.1)',
        backdropFilter: 'blur(10px)', borderRadius: 12,
        padding: '16px',
        display: 'flex', flexDirection: 'column', gap: 12,
      }}>
        <span style={{ fontSize: 13, fontWeight: 700, color: '#ccc', letterSpacing: '0.1em', textTransform: 'uppercase' }}>
          Theme Palette
        </span>
        <div style={{ display: 'grid', gridTemplateColumns: '1fr', gap: 8 }}>
          {PALETTES.map((pal, i) => (
            ...
            >
              <div style={{ width: 14, height: 14, borderRadius: '50%', background: pal.accent, flexShrink: 0 }}></div>
              <span style={{ fontSize: 12, fontWeight: 600, color: activeIdx === i ? pal.accent : '#fff' }}>
                {pal.name}
              </span>
            </button>
          ))}
